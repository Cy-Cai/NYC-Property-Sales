...
train_set %>% mutate(price_per_square_feet=SALE.PRICE/GROSS.SQUARE.FEET) %>% 
  ggplot(aes(x=fct_reorder(ZIP.CODE,price_per_square_feet,.desc=TRUE),y=price_per_square_feet))+
  geom_boxplot(outlier.colour="black", outlier.shape=16,
               outlier.size=2, notch=FALSE)
...
#Store the results 
rmse_results <-rbind(rmse_results, data.frame(method="Is Manhattan and High Pricing Area",RMSE_in_thousand=round(rmse_IsManhattanAndHighPricingArea_model/1000,0),MED_in_thousand=round(median(temp_p$r)/1000,0))%>%
                       mutate(k=round(RMSE_in_thousand/MED_in_thousand,0),Adj_r_sq=summary(fit_IsManhattanAndHighPricingArea)[9]))
